...
7. **背景素材与渲染系统重构设计（进行中）**
   - 诊断了 AI 背景素材（电线白底、架子黑底、酒瓶黑框等）抠图不净与画风撕裂的根本原因（图片带有杂色边缘且存在 Next.js 静态资源强缓存）。
   - 提出了纯 PixiJS Graphics 代码矢量化绘制方案，可彻底实现无拉伸、无边缘杂色的高保真赛博朋克极简逆光霓虹风格。
   - 已更新项目实施计划 `implementation_plan.md` 方案提请用户反馈与审批。
